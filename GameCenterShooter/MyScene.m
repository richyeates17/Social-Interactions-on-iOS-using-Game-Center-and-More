...
-(void)gameOver {
    ...
    [GameData sharedData].highScore = score;
    
    [self sendLeaderBoardData];
    
    [self sendAchievementData];
    
    [self removeAllChildren];
    
    UIViewController *gameVC = [GameData sharedData].inGameViewController;
    [gameVC.navigationController popViewControllerAnimated:YES ];
...
-(void)sendAchievementData{
    if ([GameData sharedData].isGameCenterEnabled) {
        
        GKAchievement *oneShotKOAchievement = [[GKAchievement alloc] initWithIdentifier:@"com.richardyeates.GameCenter.OneShotKO"];
        
    
        if (numberOfShots == 1 && allObjectivesDestroyed) {
            oneShotKOAchievement.percentComplete = 100;
            oneShotKOAchievement.showsCompletionBanner = YES;
        }
        
        NSArray *achievements = [NSArray arrayWithObjects:oneShotKOAchievement, nil];
...
-(void)sendLeaderBoardData{
    if ([GameData sharedData].isGameCenterEnabled) {
        ...
        GKScore *theScore = [[GKScore alloc]initWithLeaderboardIdentifier:@"com.richardyeates.GameCenter.highscore"];
        theScore.value = [GameData sharedData].highScore;
        theScore.context = 0;
        
        NSArray *scores = [NSArray arrayWithObjects:theScore, nil];
